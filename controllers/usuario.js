    ...
    Usuario.findOne({usuario: usuarioIngresado}, (err, usuarioEncontrado) => {
        if(err || usuarioEncontrado == null){
            ...
            bcrypt.compare(passwordIngresado, usuarioEncontrado.password, (err, check)=>{
                if(check){
                    Persona.findById(usuarioEncontrado.personaId, (err, personaEncontrada) => {
                        if(err){
                            resp.status(500).send({message: "No se pudo consultar a la persona."});
                        }
                        else{
                            resp.status(200).send({message: "Usuario autenticado", usuarioId: usuarioEncontrado._id, personaId: personaEncontrada._id, token : token.obtenerTokenDeUsuario(usuarioEncontrado)});
                        }
                    });
                }
                else{
